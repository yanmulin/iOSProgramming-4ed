...
-(instancetype)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        ...
        [self.tableView registerNib:nib forCellReuseIdentifier:@"BNRCourseCell"];
//        [self.tableView registerClass:BNRCourseCell.class forCellReuseIdentifier:@"BNRCourseCell"];
        [self fetchFeed];
    }
    return self;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BNRCourseCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"BNRCourseCell" forIndexPath:indexPath];
    cell.titleLabel.text = self.courses[indexPath.row][@"title"];
    NSArray *upcoming = self.courses[indexPath.row][@"upcoming"];
    if (upcoming.count > 0) {
        cell.timeLabel.text = upcoming[0][@"start_date"];
    }
    return cell;
}
