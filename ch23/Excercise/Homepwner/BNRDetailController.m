...
-(void)updateAssetType {
    BNRAssetType *assetType = self.item.assetType;
    if (!assetType) {
        self.assetTypeButton.title = @"Asset Type: None";
    } else {
        self.assetTypeButton.title = [NSString stringWithFormat:@"Asset Type: %@", assetType.labelString];
    }
}
